...
// `file.type` is just a client claim and can be forged — check the actual bytes.
...
export const POST = withApiGuard(async (request) => {
    try {
        ...
        if (!matchesSignature(buffer, (file as File).type)) {
            return NextResponse.json({ data: null, location: null, message: "File content doesn't match its declared type" }, { status: 400 });
        }
