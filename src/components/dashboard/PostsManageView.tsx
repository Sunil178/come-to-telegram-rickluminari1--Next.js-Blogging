...
export interface PostsManageSearchParams {
    page?: string;
    q?: string;
    sort?: string;
    order?: string;
...
interface PostsManageViewProps {
    basePath: string;
    heading: string;
    description?: string;
    /** Restricts the listing to one author's posts. Omit to show every author's posts. */
    scopeUserId?: string;
    /** Rendered next to the search box — e.g. "Add Post" on the self-scoped view. */
    composeControl?: React.ReactNode;
    searchParams: PostsManageSearchParams;
}

export default async function PostsManageView({
    basePath,
    heading,
    description,
    scopeUserId,
    composeControl,
    searchParams,
}: PostsManageViewProps) {
    const { page: pageParam, q, sort, order, approval, published } = searchParams;
    const page = Math.max(1, parseInt(pageParam || "1", 10) || 1);
    const sortField: SortField = SORTABLE_FIELDS.includes(sort as SortField) ? (sort as SortField) : "createdAt";
    const sortOrder = order === "asc" ? 1 : -1;

    const filter: FilterQuery<IPost> = scopeUserId ? { userId: scopeUserId } : {};
    ...
    const buildHref = (overrides: Record<string, string | number | undefined>) =>
        buildSearchParamsHref(basePath, { q, sort, order, approval, published, page: String(page), ...overrides });
    ...
    return (
        <div className="mx-auto max-w-6xl px-6 py-10">
            <h1 className="font-heading text-3xl font-semibold tracking-tight text-foreground">{heading}</h1>
