...
- ~~Create `src/middleware.ts`~~ **Done, as `src/proxy.ts`** — Next.js 16 renamed the `middleware.ts` convention to `proxy.ts` (same file location rules: project root, or inside `src/` alongside `app/`), and this file already exists using the `auth()` wrapper from `src/auth.config.ts`'s thin, DB-free config, with a matcher covering `/posts/add`, `/posts/:slug/edit`, and `/dashboard/:path*`. Confirmed live: unauthenticated requests to any of those routes redirect to `/auth/login?callbackUrl=...`.
